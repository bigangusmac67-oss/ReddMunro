...
def generate(worksheet_csv, payload, cold_exporter, primary_exporter="otlp",
    ...
    L = []
    # Line 1, before the banner. The UNDO detail further down is good
    # but it is twenty lines in, and this file gets pasted into review
    # tools that truncate. The single most reassuring fact about a
    # not-yet-applied config is that it has not been applied, and that
    # belongs where a truncated view still shows it.
    #
    # Deliberately not a git command: this generator does not know the
    # caller's repository or config path, and a restore command that is
    # wrong is worse than none, because it is run under pressure.
    if allow_drop:
        L.append("# RESTORE: NOT POSSIBLE ONCE APPLIED — --allow-drop "
                 "discards series permanently. Do not apply without a "
                 "reviewed rollback plan of your own.")
    else:
        L.append(f"# RESTORE: this file has not been applied to anything. "
                 f"Applying is your step; to undo, restore your previous "
                 f"collector config and restart. No data is deleted either "
                 f"way — routed series remain readable in "
                 f"'{cold_exporter}'.")
    L.append("#")
    L.append("# " + "=" * 68)
