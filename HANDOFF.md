...
| **Trend check fires on data with no time axis** | **Open, found by the AI corpus.** `trend_dominated` compares raw against differenced and reports a "Calendar trend confound" whenever they diverge — including on 541 language models, where there is no calendar and no row order at all. Worse than the silent mishandling it replaced: a named, confident, categorically impossible finding invites an explanation, and someone will construct one. Fix is to gate the check on a declared ordered basis; the declaration machinery exists and this check does not consult it. |
...
| **Equal-occupancy binning fails on tied data** | **Open, found by the MI cycle.** `_discretise` cannot equalise a column with fewer distinct values than bins. Poisson(4) — 13 distinct values, 12 bins — left 4 bins empty, occupancy 19–229 instead of 100, and the MI bias floor fell 61% (0.0746 → 0.0290). Integer counts are everywhere in telemetry, so the nonlinear check behaves differently on count data than on continuous data. No fix attempted; the floor is still computed per-dataset so results are internally correct, but cross-dataset comparison of MI figures is not safe. |
